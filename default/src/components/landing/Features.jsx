...
export default function Features() {
  return (
    <section className="relative py-24">
      ...
      <div className="pointer-events-none absolute left-1/2 top-1/2 h-[520px] w-[820px] -translate-x-1/2 -translate-y-1/2 rounded-full radial-crimson blur-2xl" />
        ...
        <div className="mt-14 grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
          {FEATURES.map((feature, index) => (
            <Reveal key={feature.number} delay={index * 120}>
              <div className="group relative h-full overflow-hidden rounded-2xl border border-line bg-panel p-6 transition-all duration-300 hover:border-rose-500/50 hover:-translate-y-1.5">
                <div className="flex items-start justify-between">
                  <span
                    className={`grid h-12 w-12 place-items-center rounded-xl bg-gradient-to-br ${feature.accent} text-obsidian transition-transform duration-300 group-hover:scale-110`}
                  >
