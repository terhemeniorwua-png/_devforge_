...
function CourseCard({ course, index }) {
  return (
    <Reveal delay={index * 90}>
      <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-line bg-panel transition-all duration-300 hover:border-rose-500/50 hover:-translate-y-1.5">
...
export default function CourseCatalog() {
  ...
  return (
    <section id="catalog" className="relative scroll-mt-24 py-24">
      ...
      <div className="pointer-events-none absolute -right-40 bottom-20 h-[400px] w-[400px] rounded-full radial-crimson blur-2xl" />
              ...
              {ECOSYSTEMS.map((eco) => (
                <button
                  ...
                  className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all ${
                    ecosystem === eco.key
                      ? "bg-white/10 text-white ring-1 ring-rose-500/50"
                      : "text-zinc-400 hover:bg-white/5 hover:text-white"
                  }`}
                >
                  {eco.label}
