...
function Logo() {
  return (
    <Link href="/" className="group flex items-center gap-2.5 shrink-0">
      <span className="relative grid h-9 w-9 place-items-center rounded-lg bg-gradient-to-br from-red-500 via-rose-600 to-red-900">
...
function Dropdown({ label, items, onSelect }) {
  ...
  return (
    ...
    >
      ...
      {open && (
        <div className="absolute left-0 top-full z-50 mt-2 w-48 overflow-hidden rounded-xl border border-line bg-panel/95 p-1.5 backdrop-blur-md">
          {items.map((item) => (
            <button
              key={item.label}
              type="button"
    ...
    <header className="sticky top-0 z-50 border-b border-line/70 bg-obsidian/85 backdrop-blur-xl">
      ...
      <nav className="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-3 lg:flex">
          ...
          <Link
            href="/start-learning"
            className="group relative inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-red-500 via-rose-600 to-red-800 px-4 py-2 text-sm font-semibold text-obsidian transition-all"
          >
          ...
          <div className="mx-auto max-w-7xl space-y-1 px-4 py-4 sm:px-6">
            ...
            <div className="grid grid-cols-2 gap-3 pt-4">
              ...
              <Link
                href="/start-learning"
                onClick={() => setMobileOpen(false)}
                className="rounded-lg bg-gradient-to-r from-red-500 to-rose-700 px-4 py-2.5 text-center text-sm font-semibold text-obsidian"
              >
                Start Learning
              </Link>
            </div>
